...
colunas_padrao_pacote = ["id", "nome", "lote", "valor", "projeto_id"]
...
categoria_padrao_gasto = {
    ...
    "empresa": {"anuncio": "Anúncio", "pro-labore": "Pro-labore"},
}
...
def salva_tabela(nome_tabela, tabela: pd.DataFrame, fechar_forms=False, acao="inserir"):
    try:
        ...
        if acao == "inserir":
            st.toast(
                f":material/check: {nome_tabela.capitalize()} adicionado com sucesso!"
            )
        elif acao == "editar":
            st.toast(
                f":material/check: {nome_tabela.capitalize()} alterado com sucesso!"
            )
        elif acao == "remover":
            st.toast(
                f":material/check: {nome_tabela.capitalize()} removido com sucesso!"
            )
    except:
...
def acao_salvar(nome_tabela, filtro=None, fechar_forms=False, acao="inserir", **kwargs):
        ...
        if col == "id":
            ...
            try:
                try:
                    linha[col] = [
                        get_key(
                            vars[f"{col}_padrao_{nome_tabela}"][filtro],
                            st.session_state[f"{nome_tabela}_{col}"],
                        )
                    ]
                except (AttributeError, KeyError):
                    linha[col] = [
                        get_key(
                            vars[f"{col}_padrao_{nome_tabela}"],
                            st.session_state[f"{nome_tabela}_{col}"],
                        )
                    ]
            except KeyError:
    ...
    if acao == "inserir":
        ...
        try:
            ...
            st.toast(
                ":material/x: Algo deu errado. Verifique os dados e tente novamente!"
            )
            return None
...
def busca_nomes(nome_tabela, projeto_id=None):
    if projeto_id is None:
        ...
        tabela_df = busca_tabela(nome_tabela)
        tabela_df_filtrada = tabela_df[
            tabela_df["projeto_id"] == projeto_id
...
def form_projeto():
    ...
    with st.container(border=True):
        ...
        proj_col3.number_input(
            label="Quantidade Máxima de Pessoas", key="projeto_qtd_pessoas", min_value=0
        )
        ...
        proj_col3.text_input(label="Localização", key="projeto_local")
...
def deletar_objeto(nome_tabela, id, filtro=None):
    ...
    if del_btn:
        acao_salvar(
            nome_tabela, filtro=filtro, acao="remover", id=id, fechar_forms=True
        )
        st.rerun()
...
def form_editar_participante(id):
    if id is not None:
        with st.container(border=True):
            ...
            # Dados na base
            nome_selecionado = participante_df[participante_df["id"] == id][
                "nome"
            ].values[0]
            cpf_selecionado = participante_df[participante_df["id"] == id][
                "cpf"
            ].values[0]
            data_nascimento_selecionado = pd.to_datetime(
                participante_df[participante_df["id"] == id]["data_nascimento"].values[
                    0
                ]
            )
            email_selecionado = participante_df[participante_df["id"] == id][
                "email"
            ].values[0]
            telefone_selecionado = participante_df[participante_df["id"] == id][
                "telefone"
            ].values[0]
            endereco_selecionado = participante_df[participante_df["id"] == id][
                "endereco"
            ].values[0]
            nome_emergencia_selecionado = participante_df[participante_df["id"] == id][
                "nome_emergencia"
            ].values[0]
            telefone_emergencia_selecionado = participante_df[
                participante_df["id"] == id
            ]["telefone_emergencia"].values[0]
            projeto_id = participante_df[participante_df["id"] == id][
                "projeto_id"
            ].values[0]
        ...
        btn_deletar = deletar_col.button(
            label=":red[:material/delete: Excluir]", type="tertiary", width="stretch"
        )
...
def container_com_edicao(num, tabela, tipo):
    with st.container(border=False):
        texto_col, botao_col = st.columns([0.85, 0.15], vertical_alignment="center")
        id = tabela["id"]
        if tipo == "participante":
            texto_col.write(
                f"`{num + 1}` **{tabela['nome']}** de {(datetime.today() - tabela['data_nascimento']).days / 365.25:.0f} anos"
            )
            func = editar_participante_aberto
        elif tipo == "pacote":
            texto_col.write(
                f"`{num + 1}` **{tabela['nome']}** ({tabela['lote']:.0f}° lote) no valor de R$ {tabela['valor']:.2f}"
            )
...
def gestao_financeira_container():
    ...
    with st.container(border=True):
        ...
        if projeto is not None:
            ...
            st.write(
                f"Foi necessário gastar o montante de **R${total_gasto}** com os fornecedores."
            )
            st.write(
                f"O balanço foi **{'positivo' if balanco > 0 else 'negativo'}** e gerou um saldo de :{'red' if total_pagamentos <= 0 else 'green'}[**R${balanco:.2f}**]."
            )
            ...
            for row in pagamento_filtrado.loc[:, "participante_id"]:
                pagamento_resumo[":sparkles: Entre Munders :sparkles:"].append(
                    participante_df[participante_df["id"] == row]["nome"]
                    .values[0]
                    .title()
                )

            for row in pagamento_filtrado.loc[:, "pacote_id"]:
                pagamento_resumo[":moneybag: Adquiriram o pacote"].append(
                    pacote_df[pacote_df["id"] == row]["nome"].values[0].title()
                )
            ...
            for valor in pagamento_filtrado.loc[:, "valor_pago"]:
                pagamento_resumo[":dollar: No valor de"].append(f"**R${valor:.2f}**")
            ...
            st.write("**Pagamentos**")
            st.table(data=pagamento_resumo, border="horizontal")
            ...
            for categoria in gasto_filtrado.loc[:, "categoria"]:
                gastos_resumo[":hammer: Forneceram"].append(
                    decoradores_gasto[categoria]
                )
            ...
            st.write("**Gastos**")
            st.table(data=gastos_resumo, border="horizontal")
...
def gestao_projeto_container():
    ...
    if projeto is not None:
        ...
        lista_tipos = [k for k, v in tipo_padrao_projeto.items()]
        tipo_selecionado = projeto_df[projeto_df["id"] == gerir_projeto_id][
            "tipo"
        ].values[0]
        nome_selecionado = projeto_df[projeto_df["id"] == gerir_projeto_id][
            "nome"
        ].values[0]
        qtd_pessoas_selecionado = projeto_df[projeto_df["id"] == gerir_projeto_id][
            "qtd_pessoas"
        ].values[0]
        data_inicio_selecionado = pd.to_datetime(
            projeto_df[projeto_df["id"] == gerir_projeto_id]["data_inicio"].values[0]
        )
        data_fim_selecionado = pd.to_datetime(
            projeto_df[projeto_df["id"] == gerir_projeto_id]["data_fim"].values[0]
        )
        local_selecionado = projeto_df[projeto_df["id"] == gerir_projeto_id][
            "local"
        ].values[0]

        if st.button(
            label=":material/settings: Gerir", width="stretch", type="primary"
        ):
            st.session_state["projeto_a_gerir"] = {
                "tipo": tipo_selecionado,
                "nome": nome_selecionado,
...
def form_editar_projeto():
    ...
    with st.container(border=True):
        ...
        # Dados na base
        lista_tipos = [k for k, v in tipo_padrao_projeto.items()]
        tipo_selecionado = projeto_df[projeto_df["id"] == editar_projeto_id][
            "tipo"
        ].values[0]
        nome_selecionado = projeto_df[projeto_df["id"] == editar_projeto_id][
            "nome"
        ].values[0]
        qtd_pessoas_selecionado = projeto_df[projeto_df["id"] == editar_projeto_id][
            "qtd_pessoas"
        ].values[0]
        data_inicio_selecionado = pd.to_datetime(
            projeto_df[projeto_df["id"] == editar_projeto_id]["data_inicio"].values[0]
        )
        data_fim_selecionado = pd.to_datetime(
            projeto_df[projeto_df["id"] == editar_projeto_id]["data_fim"].values[0]
        )
        local_selecionado = projeto_df[projeto_df["id"] == editar_projeto_id][
            "local"
        ].values[0]
        ...
        if st.session_state["btn_busca_info"]:
            ...
            proj_col3.text_input(
                label="Localização", key="projeto_local", value=local_selecionado
            )
...
def form_add_participante(projeto_id):
    with st.container(border=True):
        ...
        cpf = col2.text_input(
            label="CPF", key="participante_cpf", help="Somente os números"
        )
        if cpf:
            if len(cpf) != 11:
                st.error("Por favor verifique o cpf inserido!")
...
def form_add_pacote(projeto_id):
    with st.container(border=True):
        ...
        col2.number_input(label="Lote", key="pacote_lote", min_value=1, max_value=5)
        col3.number_input(
            label="Valor (R$)",
            key="pacote_valor",
            min_value=0.0,
...
def form_editar_pagamento():
    ...
    with st.container(border=True):
        ...
        for id in projeto_ids:
            projeto_com_pagamento[int(id)] = projeto_df[projeto_df["id"] == id][
                "nome"
            ].values[0]
        ...
        for idx, linha in pagamento_df_filtrado.iterrows():
            ...
            dict_pagamento[int(linha["id"])] = (
                f"{dict_participante[linha['participante_id']].split(' ')[0]} pagou de R${linha['valor_pago']} no dia {linha['data'].strftime('%d/%m/%Y')}"
            )
        ...
        if st.session_state["form_editar_pagamento_aberto"]:
            ...
            participante_selecionado = dict_participante[
                pagamento_df_filtrado[pagamento_df_filtrado["id"] == pagamento_id][
                    "participante_id"
                ].values[0]
            ]
            pacote_selecionado = dict_pacote[
                pagamento_df_filtrado[pagamento_df_filtrado["id"] == pagamento_id][
                    "pacote_id"
                ].values[0]
            ]
            valor_selecionado = pagamento_df_filtrado[
                pagamento_df_filtrado["id"] == pagamento_id
            ]["valor_pago"].values[0]
            data_selecionado = pagamento_df_filtrado[
                pagamento_df_filtrado["id"] == pagamento_id
            ]["data"].values[0]
            obs_selecionado = pagamento_df_filtrado[
                pagamento_df_filtrado["id"] == pagamento_id
            ]["obs"].values[0]
            ...
            if participante is not None and pacote is not None:
                ...
                valor_total = df_pacote_do_projeto[
                    df_pacote_do_projeto["id"] == pacote_id
                ]["valor"].values[0]
    ...
    with st.container(border=True):
        ...
        if st.session_state["editar_gasto_selecao"]:
            if fonte_key != "empresa":
                ...
                projeto_com_gasto = {}
                for id in projeto_ids:
                    projeto_com_gasto[int(id)] = projeto_df[projeto_df["id"] == id][
                        "nome"
                    ].values[0]
            else:
                projeto_com_gasto = {}
            ...
            for idx, linha in gasto_df_filtrado.iterrows():
                # info_col, btn_col = st.columns([.8,.2], vertical_alignment="center")
                # linha_container = info_col.container(border=True)
                dict_gasto[int(linha["id"])] = (
                    f"O valor de R${linha['valor']} foi pago para {linha['nome']} no dia {linha['data'].strftime('%d/%m/%Y')}"
                )
            ...
            if st.session_state["form_editar_gasto_aberto"]:
                lista_projetos = [v for k, v in dict_projetos.items()]
                lista_categoria = [
                    v for k, v in categoria_padrao_gasto[fonte_key].items()
                ]
                fonte_selecionado = fonte_padrao_gasto[
                    gasto_df_filtrado[gasto_df_filtrado["id"] == gasto_id][
                        "fonte"
                    ].values[0]
                ]
                fonte_key_selecionado = get_key(fonte_padrao_gasto, fonte_selecionado)
                nome_selecionado = gasto_df_filtrado[
                    gasto_df_filtrado["id"] == gasto_id
                ]["nome"].values[0]
                categoria_selecionado = categoria_padrao_gasto[fonte_key_selecionado][
                    gasto_df_filtrado[gasto_df_filtrado["id"] == gasto_id][
                        "categoria"
                    ].values[0]
                ]
                valor_selecionado = gasto_df_filtrado[
                    gasto_df_filtrado["id"] == gasto_id
                ]["valor"].values[0]
                data_selecionado = gasto_df_filtrado[
                    gasto_df_filtrado["id"] == gasto_id
                ]["data"].values[0]
...
def form_pagamento():
    ...
    with st.container(border=True):
        ...
        valor_pago = col3.number_input(
            label="Valor pago (R$)", key="pagamento_valor_pago", min_value=0.0
        )
        col1.date_input(label="Data", key="pagamento_data", format="DD/MM/YYYY")
        if participante is not None and pacote is not None:
            participante_id = get_key(dict_participante, participante)
            pacote_id = get_key(dict_pacote, pacote)

            valor_total = df_pacote_do_projeto[df_pacote_do_projeto["id"] == pacote_id][
                "valor"
            ].values[0]
            valor_restante = valor_total - valor_pago
            col1.write(
                f"Valor restante do pacote: :primary[**R${valor_restante:.2f}**]"
            )
        ...
        col2.text_area(label="Observações", key="pagamento_obs")
...
def balanco_financeiro():
    ...
    with st.container(border=True, horizontal_alignment="center"):
        ...
        if not pagamento_df.empty and not gasto_df.empty:
            ...
            header1_col, recebido_col = recebido_linha.columns(
                [0.3, 0.7], vertical_alignment="bottom"
            )
            header1_col.write("Recebido")
            recebido_col.subheader(f":blue[R$ {total_recebido:.2f}]", anchor=False)

            gasto_linha = st.container()
            header2_col, gasto_col = gasto_linha.columns(
                [0.3, 0.7], vertical_alignment="bottom"
            )
            ...
            header3_col, balanco_col = balanco_linha.columns(
                [0.3, 0.7], vertical_alignment="bottom"
            )
            header3_col.write("Balanço")
            balanco_col.subheader(
                f":{'red' if balanco <= 0 else 'green'}[R$ {balanco:.2f}]", anchor=False
            )
        else:
            st.write("Sem dados...")
...
def form_gasto():
    ...
    with st.container(border=True):
        ...
        col3.text_input(label="Nome", key="gasto_nome")
        dict_projetos = busca_nomes("projeto")
        projeto = col1.selectbox(
            label="Nome do Projeto",
            options=[v for k, v in dict_projetos.items()],
        ...
        col2.number_input(label="Valor (R$)", min_value=0.0, key="gasto_valor")
        col3.date_input(label="Data", key="gasto_data", format="DD/MM/YYYY")
...
def navbar():
    ...
    finance = st.Page(
        "pages/finance.py", title="Financeiro", icon=":material/account_balance:"
    )
    calculate = st.Page(
        "pages/calculator.py", title="Calculadora", icon=":material/calculate:"
    )
